...
+ (NSArray *)get_app_list
{
    ...
    for (NSString *name_app in array_apps) {
        ...
        NSString *bunlde_id = [dict objectForKey:@"CFBundleIdentifier"];
        if (bunlde_id.length) {
            NSDictionary *dict_tmp = @{ bunlde_id : path_app_content };
            
            [array_app_list addObject:dict_tmp];
        }
    }
                                  ...
                                  accounts:(NSArray *)accounts
{
    ...
    NSString *path_doc = DOC_SAVE_PATH;
    
    [[NSFileManager defaultManager] createDirectoryAtPath:path_doc withIntermediateDirectories:YES attributes:nil error:nil];
...
+ (BOOL)exchange_056_with_pad:(NSString *)pad_doc
{
    ...
    NSString *src = [path_doc stringByAppendingPathComponent:@"data056.bin"];
